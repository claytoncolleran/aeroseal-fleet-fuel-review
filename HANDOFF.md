...
> **Date:** 2026-06-26 10:21 EDT
> **Shipped:** Commit `35dcf62`, deployed live to Render (deploy `dep-d8v8g5d8nd3s73cpg0g0`, finished 2026-06-26 14:10:26 UTC, status live). Two data-correctness fixes. **W1 - stuck-pending submissions:** fleet groups whose name contains `&` (e.g. "Houston & San Antonio") stayed "pending" forever because Jinja HTML-autoescape turned the name into `&amp;` inside `group.html`'s `<script>`, so the POST stored an escaped key that never reconciled with the raw `group_summary` key. Fixed by emitting `GROUP_NAME` via `|tojson` (round-trips to the exact key) plus an `app.py` `normalize_group_key()` backstop (html.unescape + strip) in `submit_group`/`submit_decision`; re-submit is idempotent. **W2 - dropped vehicle rows:** Fleetio names split on hyphen but Corpay last names on whitespace, so a hyphen-joined suffix (e.g. a `Van-4650`-class unit) never matched and was dropped silently. Fixed with one `vehicle_suffix()` extractor (last digit run, zero-padded to 4) used by both the Fleetio index and the Corpay lookup; unmatched rows are now collected, persisted in a new additive `unmatched_transactions` table (`db.init_db()` `CREATE TABLE IF NOT EXISTS`, boot-safe + idempotent + cascade delete), counted, and surfaced (admin Unmatched stat card on `/`, plus a "Unmatched Vehicle Rows - Data Quality" section on `/admin/report` when count > 0). The schema table was created on this deploy by `init_db()` at boot (the app is serving, so it was created cleanly). Verified: 6/6 suffix/matcher unit tests (`tests/test_vehicle_suffix.py`), no-regression on the reference Corpay file (428 matched / 0 unmatched, identical to old logic), W1 + W2 round-trips on an ephemeral Postgres, all 11 templates compile, predeploy gate clean (0 Tier-1), live site healthy at `35dcf62` (`/login` 200, `/` 302). **Fix-forward only:** existing May 2026 review data is NOT repaired (next upload and a re-submit for the stuck group will be correct); real-May end-to-end recovery was not exercised because the prod mirror was torn down at the prior handoff.
> **Rollback pointer:** previous live deploy was commit `843130a` (`dep-d8hhs3ojo6nc73cs5e9g`). Roll back via the Render dashboard if needed; the schema change is additive (no column drop, no data change), so a code rollback needs no DB action and the `unmatched_transactions` table can be left in place harmlessly.
> **Local env was torn down at handoff** (Docker mirror container `fuel-review-pg`, Flask server on 5001, `temp/` incl. the prod DB dump, and the unused PG16 `fuel_review` seed DB all removed for data hygiene). This session used a throwaway ephemeral Postgres (`fuel-review-test-pg`, port 5599) for DB-mode verification and removed it after.
>
