...
- Data-correctness fixes (2026-06-26, commit `35dcf62`): **(W1) stuck-pending submissions** - groups whose name contains `&` (e.g. "Houston & San Antonio") could never be marked submitted because Jinja HTML-autoescape stored an `&amp;` key that never reconciled with the raw `group_summary` key; fixed by emitting `GROUP_NAME` via `|tojson` in `group.html` plus a `normalize_group_key()` (html.unescape + strip) backstop in `submit_group`/`submit_decision`, with idempotent re-submit. **(W2) dropped vehicle rows + surfaced unmatched** - replaced the two inconsistent suffix extractors (Fleetio split on hyphen, Corpay on whitespace) with one `vehicle_suffix()` (last digit run, zero-padded to 4) used by both, recovering hyphen-joined suffixes like `Van-4650` without regressing existing matches; unmatched vehicle rows are now collected, stored in a new additive `unmatched_transactions` table (boot-safe, idempotent, cascade delete), counted, and surfaced (admin Unmatched stat card + a data-quality section on `/admin/report`). Fix-forward only (existing May 2026 data not repaired). Suffix unit tests in `tests/test_vehicle_suffix.py`: COMPLETE

## Roadmap / Open Items

### Known data note: missing April 2026 review (logged 2026-06-05)
Production has only two reviews: `2026-03` (complete) and `2026-05` (in_review). There is **no April review** and **zero transactions dated Apr 1-28** in the DB. The March review covers the Corpay cycle Feb 27-Mar 30; the May review covers Apr 29-May 30, so the April billing cycle (~Mar 31-Apr 28) was never processed/retained. The `reviews` id sequence jumped 7 -> 18 (ten ids consumed and deleted), consistent with April attempts that were created and later deleted. NOTE: `delete_review` in DB mode removes only DB rows, not the persistent-disk files (app.py:1464-1477), so a deleted April review's `corpay_upload.xlsx` / `anomaly_report.json` may still exist under `/var/data/reviews/2026-04/`. To resolve: either re-run the April Corpay cycle as a new review, or (if recovery is wanted) check that disk path. Separately, **unmatched vehicle rows used to be dropped silently** during processing (`match_transactions`) - not stored, not surfaced, only console-logged. RESOLVED 2026-06-26 (commit `35dcf62`): unmatched rows are now collected, persisted in the `unmatched_transactions` table, counted, and surfaced (admin Unmatched stat card + a data-quality section on the consolidated report). The matcher was also hardened (single `vehicle_suffix()` extractor) so hyphen-joined Corpay suffixes no longer miss in the first place.
